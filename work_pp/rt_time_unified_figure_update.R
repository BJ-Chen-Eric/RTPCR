...
make_gene_faceted_plot <- function(gene_name, keys, plot_df_list, result_time_list, sample_order, control_time,
                                   gene_fill_color_map, bar_fill_color, gg_theme) {
  keys <- keys[keys %in% names(plot_df_list)]
  ...
  for (k in keys) {
    meta <- header_cleaning(k, "_")
    if (nrow(meta) == 0) next
    strain_name <- as.character(meta$V1[1])

    plot_df <- plot_df_list[[k]]
    ...
    plot_df <- plot_df %>%
      mutate(
        ...
        time_label = stringr::str_extract(as.character(p), "[0-9]+h$"),
        time_label = ifelse(is.na(time_label), as.character(p), time_label),
        mean_label = sprintf("%.2f (n=%d)", mean_fc, n_points)
      ) %>%
      arrange(ifelse(str_detect(p, paste0("_", control_time, "$")), -Inf, time_num), p) %>%
      mutate(panel_order = row_number(), panel_key = paste(strain, as.character(p), sep = "_"))
    sum_parts[[k]] <- plot_df

    raw_df <- result_time_list[[k]] %>%
      mutate(
        fold_change_num = suppressWarnings(as.numeric(fold_change)),
        strain = strain_name,
        panel_key = paste(strain, as.character(p), sep = "_")
      ) %>%
      filter(is.finite(fold_change_num))
    raw_parts[[k]] <- raw_df

    # Preserve star text per facet/panel (layout-only change: text retained, no bracket line).
    if (nrow(plot_df) == 2) {
      pval <- safe_t_test_p(result_time_list[[k]], group_col = "p")
      stars_parts[[k]] <- plot_df %>%
        slice(n()) %>%
        mutate(stars = p_to_stars(pval), stars_y = max(stars_y, na.rm = TRUE) + 0.05)
      ...
      if (!is.na(control_label)) {
        ...
        for (p_lab in as.character(plot_df$p)) {
          ...
          sub_df <- result_time_list[[k]] %>% filter(p %in% c(as.character(control_label), p_lab))
        ...
        if (length(stars_map) > 0) {
          stars_parts[[k]] <- plot_df %>%
            ...
            mutate(stars = unname(stars_map[as.character(p)]), stars_y = stars_y + 0.05)
        }
      }
    }
  }

  plot_df_all <- dplyr::bind_rows(sum_parts)
  if (nrow(plot_df_all) == 0) return(NULL)
  raw_points_all <- dplyr::bind_rows(raw_parts)
  stars_df_all <- if (length(stars_parts) > 0) dplyr::bind_rows(stars_parts) else data.frame()

  strain_levels <- unique(c(sample_order, unique(as.character(plot_df_all$strain))))
  strain_levels <- strain_levels[strain_levels %in% unique(as.character(plot_df_all$strain))]
  if (length(strain_levels) == 0) strain_levels <- unique(as.character(plot_df_all$strain))
  # Force display order: Local (L) then Systemic (S), then others.
  local_lv <- strain_levels[tolower(strain_levels) == "l"]
  systemic_lv <- strain_levels[tolower(strain_levels) == "s"]
  other_lv <- strain_levels[!(tolower(strain_levels) %in% c("l", "s"))]
  strain_levels <- c(local_lv, systemic_lv, other_lv)

  plot_df_all <- plot_df_all %>%
    mutate(strain = factor(as.character(strain), levels = strain_levels)) %>%
    arrange(strain, panel_order)

  panel_levels <- plot_df_all %>%
    arrange(strain, panel_order) %>%
    pull(panel_key) %>%
    as.character() %>%
    unique()
  plot_df_all$panel_key <- factor(as.character(plot_df_all$panel_key), levels = panel_levels)
  time_labels <- plot_df_all %>%
    dplyr::distinct(panel_key, time_label) %>%
    mutate(panel_key = as.character(panel_key), time_label = str_remove(as.character(time_label), pattern = "h$"))
  x_labels <- setNames(time_labels$time_label, time_labels$panel_key)
  raw_points_all <- raw_points_all %>%
    mutate(
      strain = factor(as.character(strain), levels = strain_levels),
      panel_key = factor(as.character(panel_key), levels = panel_levels)
    )
  if (nrow(stars_df_all) > 0) {
    stars_df_all <- stars_df_all %>%
      mutate(
        strain = factor(as.character(strain), levels = strain_levels),
        panel_key = factor(paste(as.character(strain), as.character(p), sep = "_"), levels = panel_levels)
      )
  }

  ymax <- max(plot_df_all$mean_fc + plot_df_all$sd_fc, na.rm = TRUE)
  if (!is.finite(ymax) || ymax <= 0) ymax <- 1
  y_max_int <- round(ymax * 1.2, digit = 1)

  bw_fill_values <- setNames(rep(c("black", "white"), length.out = length(strain_levels)), strain_levels)
  strain_label_values <- setNames(
    ifelse(
      tolower(strain_levels) == "l", "local",
      ifelse(tolower(strain_levels) == "s", "syst", strain_levels)
    ),
    strain_levels
  )

  p <- ggplot(plot_df_all, aes(x = panel_key, y = mean_fc, fill = strain)) +
    geom_col(width = 0.6, color = "black") +
    geom_point(
      data = raw_points_all,
      aes(x = panel_key, y = fold_change_num),
      inherit.aes = FALSE,
    ...
    scale_fill_manual(values = bw_fill_values, labels = strain_label_values, drop = FALSE) +
    scale_x_discrete(labels = x_labels) +
    scale_y_continuous(
      limits = c(0, y_max_int+.5),
      breaks = integer_axis_breaks(0, y_max_int, 8),
      expand = expansion(mult = c(0, 0.03))
    ) +
    labs(x = "Time after treatment (Hour)", y = "Relative gene expression", fill='', title = paste(gene_name)) +
  ...
  if (nrow(stars_df_all) > 0) {
    p <- p + geom_text(
      data = stars_df_all,
      aes(x = panel_key, y = stars_y, label = stars),
      inherit.aes = FALSE,
      size = 10
    )
  }
...
main <- function() {
  ...
  for (idx in seq_along(selected_file_paths)) {
    ...
    A <- A %>%
      ...
      select(Sample_name, Sample_type, has_time_token, Time_raw, Time, Ct, Gene, p, p_filter)
  ...
  # After calibration and before data organization: remove Standard rows.
  for (nm in names(filter_csv)) {
    before_n <- nrow(filter_csv[[nm]])
    filter_csv[[nm]] <- filter_csv[[nm]] %>%
      dplyr::filter(tolower(trimws(as.character(Sample_type))) != "standard")
    removed_n <- before_n - nrow(filter_csv[[nm]])
    if (removed_n > 0) {
      message("[filter] ", nm, ": removed ", removed_n, " Standard rows (post-calibration)")
    }
  }
  ...
  if (length(result_time_list) == 0) {
    ...
    if (nrow(fallback_df) > 0) {
      ...
      message("[info] Fallback strain figures skipped (keeping only Gene* and pairwise significance figures).")
    } else {
  ...
  for (m in miR_for_combine) {
    keys_m <- names(plot_list)[grep(m, names(plot_list))]
    panel_names <- panel_name_from_result_key(keys_m, mode = analysis_mode, perspective = "miR")
    if (length(sample_order) > 0 && length(keys_m) == length(panel_names)) {
      ord_idx <- match(unique(c(sample_order, panel_names)), panel_names)
      ord_idx <- ord_idx[!is.na(ord_idx)]
      keys_m <- keys_m[ord_idx]
    }
    combine_plot_by_mir_n[[m]] <- length(keys_m)
    combine_plot_by_mir[[m]] <- make_gene_faceted_plot(
      gene_name = as.character(m),
      keys = keys_m,
      plot_df_list = plot_df_list,
      result_time_list = result_time_list,
      sample_order = sample_order,
      control_time = control_time,
      gene_fill_color_map = gene_fill_color_map,
  ...
  for (nm in names(combine_plot_by_mir)) {
    ...
    n_panels <- combine_plot_by_mir_n[[nm]]
    w <- max(16, 5 * n_panels)
    h <- 6
    save_plot_if_nonempty(combine_plot_by_mir[[nm]], file.path(figure_dir, paste0("Gene_", nm_safe, "_combined.png")), width = w, height = h)
  }

  # Also export one merged gene figure (e.g., ARF5 + ARF6) in a single row.
  gene_merge_plots <- combine_plot_by_mir[!vapply(combine_plot_by_mir, is.null, logical(1))]
  if (length(gene_merge_plots) > 0) {
    shared_legend <- cowplot::get_legend(
      gene_merge_plots[[1]] + theme(
        legend.position = "right",
        legend.justification = "center"
      )
    )

    if (length(gene_merge_plots) > 1) {
      gene_merge_plots <- lapply(seq_along(gene_merge_plots), function(i) {
        p <- gene_merge_plots[[i]]
        p + theme(legend.position = "none")
      })
    } else {
      gene_merge_plots <- lapply(gene_merge_plots, function(p) p + theme(legend.position = "none"))
    }
    gene_panel_ncol <- length(gene_merge_plots)
    gene_merge_core <- cowplot::plot_grid(plotlist = gene_merge_plots, ncol = gene_panel_ncol)
    # Width should follow the number of x-axis bar columns (panel_key), not just gene panel count.
    total_bar_cols <- sum(vapply(gene_merge_plots, function(p) {
      d <- p$data
      if (is.null(d) || !("panel_key" %in% names(d))) return(0L)
      length(unique(as.character(d$panel_key)))
    }, integer(1)))
    if (!is.finite(total_bar_cols) || total_bar_cols <= 0) total_bar_cols <- gene_panel_ncol
    gene_merge_plot <- if (is.null(shared_legend)) {
      gene_merge_core
    } else {
      # Keep legend close to plot; add trailing spacer as outer right margin.
      cowplot::plot_grid(gene_merge_core, shared_legend, NULL, nrow = 1, rel_widths = c(1, 0.14, 0.08))
    }
    save_plot_if_nonempty(
      gene_merge_plot,
      file.path(figure_dir, "Gene_merged_row1.png"),
      width = 2 * total_bar_cols,
      height = max(9, 9 * length(gene_merge_plots))
    )
  }

  # Skip strain-combined figure outputs; keep only Gene* and pairwise significance figures.
