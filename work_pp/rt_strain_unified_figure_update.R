...
print_usage <- function() {
  cat(
    "Usage:\n",
    "  Rscript rt_pct_strain_source_unified_by_strain.R [options]\n\n",
    "Options:\n",
    ...
    "  --reference-strain S    Optional reference strain for stars/order in strain plots (default: B73 if present)\n",
...
extract_strain_from_p <- function(p) {
  p <- as.character(p)
  sub("_[0-9]+h$", "", p)
}

extract_hr_from_p <- function(p) {
  p <- as.character(p)
  suppressWarnings(as.numeric(stringr::str_extract(p, "[0-9]+(?=h$)")))
}
...
main <- function() {
  ...
  reference_strain <- if (!is.null(args$`reference-strain`)) normalize_sample_name(args$`reference-strain`) else ""
  ...
  if (analysis_mode != "multi" || multi_compare_style != "all_time") {
    message("[info] by_strain pipeline forces analysis_mode=multi and multi_compare_style=all_time.")
  }
  analysis_mode <- "multi"
  multi_compare_style <- "all_time"
  ...
  message("[config] reference_strain: ", ifelse(nzchar(reference_strain), reference_strain, "<auto>"))
  ...
  for (idx in seq_along(selected_file_paths)) {
    ...
    if (!any(A$has_time_token, na.rm = TRUE)) {
      message(
        "[warn] No '<number>h' token found in file ", basename(fp),
        ". Treating all rows as time='", control_time, "' for strain comparison."
      )
    }
  ...
  if (!nzchar(reference_strain)) {
    if (any(sample == "b73")) {
      reference_strain <- "b73"
    } else if (length(sample) > 0) {
      reference_strain <- sort(sample)[1]
    } else {
      reference_strain <- ""
    }
  }
  if (nzchar(reference_strain) && !(reference_strain %in% sample)) {
    message("[warn] --reference-strain '", reference_strain, "' not found in data. Stars will use first strain per panel.")
  } else if (nzchar(reference_strain)) {
    message("[config] resolved reference_strain: ", reference_strain)
  }
  ...
  if (analysis_mode == "multi" && length(time_points) < 1) {
    stop("analysis-mode=multi requires at least 1 time point.")
  }
  compare_times <- setdiff(time_points, control_time)
  if (length(compare_times) == 0) compare_times <- time_points
  ...
  write_workbook_if_nonempty(result_raw_time, file.path(out_dir, "all_raw_strain_results.xlsx"))
  ...
  # Build strain-comparison input from the same filtered result sheets.
  strain_cmp_input <- data.frame()
  if (length(result_time_list) > 0) {
    cmp_list <- list()
    ...
    for (nm in names(result_time_list)) {
      ...
      cmp_list[[nm]] <- df %>%
        transmute(
          gene = gene_name,
          p = as.character(p),
          strain = extract_strain_from_p(p),
          hr = suppressWarnings(as.numeric(hr)),
          time_label = ifelse(is.finite(hr), paste0(as.integer(hr), "h"), NA_character_),
          data_resoure = as.character(data_resoure),
          rep = suppressWarnings(as.integer(rep)),
          con = if ("con" %in% colnames(df)) suppressWarnings(as.numeric(df$con)) else NA_real_,
          gene_ct = if ("gene" %in% colnames(df)) suppressWarnings(as.numeric(df$gene)) else NA_real_,
          delta = if ("delta" %in% colnames(df)) suppressWarnings(as.numeric(df$delta)) else NA_real_,
          pre_fold_change = if ("pre_fold_change" %in% colnames(df)) suppressWarnings(as.numeric(df$pre_fold_change)) else NA_real_,
          control = if ("control" %in% colnames(df)) suppressWarnings(as.numeric(df$control)) else NA_real_,
          fold_change = suppressWarnings(as.numeric(fold_change))
        )
    }
    if (length(cmp_list) > 0) {
      strain_cmp_input <- dplyr::bind_rows(cmp_list) %>%
        filter(is.finite(hr), is.finite(fold_change), nzchar(strain), nzchar(gene)) %>%
        distinct(gene, strain, p, data_resoure, rep, hr, fold_change, .keep_all = TRUE)
    }
  }
  # If time-comparison sheets are empty (e.g. single timepoint input), build directly from all_time.
  if (nrow(strain_cmp_input) == 0 && nrow(all_time) > 0) {
    direct_df <- all_time %>%
      transmute(
        gene_name = as.character(miR),
        p = as.character(p),
        ...
        con = suppressWarnings(as.numeric(con)),
        gene_ct = suppressWarnings(as.numeric(gene))
      ) %>%
      filter(is.finite(hr), is.finite(delta), is.finite(con), is.finite(gene_ct), con < 40, gene_ct < 40, nzchar(strain), nzchar(gene_name))

    # Keep the same rule/outlier behavior per strain-gene-time before normalization.
    direct_groups <- direct_df %>% group_by(gene_name, strain, hr, time_label) %>% group_split()
    filtered_groups <- lapply(direct_groups, function(df_g) {
      if (nrow(df_g) == 0) return(df_g)
      df_g <- apply_subset_topn_rule(
        df = as.data.frame(df_g),
        rules = subset_topn_rules,
        sample_name = as.character(df_g$strain[1]),
        gene_name = as.character(df_g$gene_name[1]),
        time_num = as.numeric(df_g$hr[1]),
        analysis_mode = "multi",
        multi_compare_style = "all_time"
      )
      if (nrow(df_g) == 0) return(df_g)
      df_g$dis <- df_g$delta - median(df_g$delta)
      df_g <- df_g %>%
        arrange(abs(dis)) %>%
        mutate(
          center = mean(delta, na.rm = TRUE),
        ...
        distinct(rep, .keep_all = TRUE) %>%
        select(-c(center, delta_sd, z_score, n, dis))
      drop_outliers_by_group(as.data.frame(df_g), max_sample = max_sample)
    })
    filtered_groups <- filtered_groups[vapply(filtered_groups, nrow, integer(1)) > 0]
    if (length(filtered_groups) > 0) {
      direct_df <- dplyr::bind_rows(filtered_groups)
      direct_df <- direct_df %>%
        mutate(
          pre_fold_change = 2^(-delta)
        ) %>%
        group_by(gene_name, hr, time_label) %>%
        mutate(
          ref_strain_here = ifelse(reference_strain %in% strain, reference_strain, sort(unique(strain))[1]),
          control = mean(pre_fold_change[strain == ref_strain_here], na.rm = TRUE),
          fold_change = pre_fold_change / control
        ) %>%
        ungroup() %>%
        filter(is.finite(fold_change))

      strain_cmp_input <- direct_df %>%
        transmute(
          gene = gene_name,
          p = p,
          strain = strain,
          ...
          data_resoure = data_resoure,
          rep = rep,
          con = con,
          gene_ct = gene_ct,
          delta = delta,
          pre_fold_change = pre_fold_change,
          control = control,
  ...
  append_execution_log(
    "strain_cmp_input_rows=", nrow(strain_cmp_input),
    ", strain_result_sheets_prebuild=", ifelse(nrow(strain_cmp_input) > 0, ">=1", "0")
  )

  strain_result_list <- list()
  if (nrow(strain_cmp_input) > 0) {
    combo <- strain_cmp_input %>%
      group_by(gene, hr, time_label) %>%
      group_split()
    for (df_g in combo) {
      if (nrow(df_g) == 0) next
      g <- as.character(df_g$gene[1])
      t_lab <- as.character(df_g$time_label[1])
      key <- paste("straincmp", g, t_lab, sep = "_")
      sheet_df <- df_g %>%
        mutate(
          dup_resource = "",
          note = "",
          con = dplyr::coalesce(suppressWarnings(as.numeric(con)), NA_real_),
          gene = dplyr::coalesce(suppressWarnings(as.numeric(gene_ct)), NA_real_),
          delta = dplyr::coalesce(suppressWarnings(as.numeric(delta)), NA_real_),
          pre_fold_change = dplyr::coalesce(suppressWarnings(as.numeric(pre_fold_change)), NA_real_),
          control = dplyr::coalesce(suppressWarnings(as.numeric(control)), NA_real_)
        ) %>%
        group_by(p) %>%
        mutate(mean_fc = if_else(row_number() == 1, mean(fold_change, na.rm = TRUE), NA_real_)) %>%
        ungroup() %>%
        select(p, data_resoure, dup_resource, note, rep, con, gene, hr, delta, pre_fold_change, control, fold_change, mean_fc, strain) %>%
        mutate(
          .ref_first = if (nzchar(reference_strain)) as.integer(as.character(strain) != reference_strain) else 0L
        ) %>%
        arrange(.ref_first, p, rep) %>%
        select(-.ref_first) %>%
        group_by(p) %>%
        mutate(mean_fc = if_else(row_number() == dplyr::n(), mean(fold_change, na.rm = TRUE), NA_real_)) %>%
        ungroup()
      strain_result_list[[key]] <- sheet_df
    }
  }
  write_workbook_if_nonempty(strain_result_list, file.path(out_dir, "all_strain_results.xlsx"))
  append_execution_log("strain_result_sheets=", length(strain_result_list))
  ...
  pair_idx <- 1
  if (nrow(strain_cmp_input) > 0) {
    cmp_groups <- strain_cmp_input %>%
      group_by(gene, hr, time_label) %>%
      group_split()

    for (df_g in cmp_groups) {
      strains <- sort(unique(df_g$strain))
      if (length(strains) < 2) next
      comb <- utils::combn(seq_along(strains), 2)
      if (is.null(dim(comb)) || ncol(comb) == 0) next

      gene_name <- as.character(df_g$gene[1])
      t_lab <- as.character(df_g$time_label[1])
      ...
      for (k in seq_len(ncol(comb))) {
        ...
        s1 <- strains[i1]
        s2 <- strains[i2]
        x <- df_g$fold_change[df_g$strain == s1]
        y <- df_g$fold_change[df_g$strain == s2]
      ...
      m <- matrix(NA_real_, nrow = length(strains), ncol = length(strains), dimnames = list(strains, strains))
      for (k in seq_len(nrow(group_df))) {
        a <- group_df$strain_a[k]
        b <- group_df$strain_b[k]
        p <- group_df$p_value[k]
        m[a, b] <- p
        m[b, a] <- p
      }
      pairwise_matrix_list[[paste(gene_name, t_lab, sep = "_")]] <- as.data.frame(m, stringsAsFactors = FALSE)
      ...
      for (k in seq_len(nrow(group_df))) {
        ...
        heat_df$p_value[heat_df$strain_a == a & heat_df$strain_b == b] <- p
        heat_df$p_value[heat_df$strain_a == b & heat_df$strain_b == a] <- p
        heat_df$p_stars[heat_df$strain_a == a & heat_df$strain_b == b] <- s
        heat_df$p_stars[heat_df$strain_a == b & heat_df$strain_b == a] <- s
      }
      heatmap_rows[[paste(gene_name, t_lab, sep = "_")]] <- heat_df
    }
  }

  if (length(pairwise_rows) > 0) {
    pairwise_table <- dplyr::bind_rows(pairwise_rows) %>%
      arrange(gene, time, strain_a, strain_b)
    utils::write.csv(pairwise_table, file.path(out_dir, "pairwise_strain_tests.csv"), row.names = FALSE)
    write_workbook_if_nonempty(pairwise_matrix_list, file.path(out_dir, "pairwise_strain_test_matrix.xlsx"))
    ...
    if (length(heatmap_rows) > 0) {
      ...
      time_levels <- sort(unique(heatmap_df$time))
      ...
      nrow_grid <- length(time_levels)
      ncol_grid <- length(gene_levels)
      heatmap_plot <- ggplot(heatmap_df, aes(x = strain_a, y = strain_b, fill = p_plot)) +
        geom_tile(color = "white", linewidth = 0.3) +
        geom_text(aes(label = p_stars), size = 3.8) +
        ...
        labs(x = "Strain", y = "Strain", fill = "p-value\n", title = "Pairwise Strain Tests by Time and Gene") +
        theme_bw() + gg_theme +
        theme(axis.text.x = element_text(angle = 45, hjust = 1), axis.title.x = element_blank(), axis.title.y = element_blank(), strip.text.y = element_text(angle = 0), panel.grid = element_blank())
      save_plot_if_nonempty(
        heatmap_plot,
        file.path(figure_dir, "pairwise_strain_test_heatmap.png"),
        width = max(10, 3.8 * ncol_grid),
        height = max(6, 3.8 * nrow_grid)
      )
    }
    append_execution_log("pairwise_strain_tests_rows=", nrow(pairwise_table), ", pairwise_groups=", length(pairwise_matrix_list))
  } else {
    message("[warn] Pairwise strain tests skipped: insufficient data across strains.")
    append_execution_log("pairwise_strain_tests_rows=0, pairwise_groups=0")
  }
  # Remove deprecated time-alias files from older by_strain runs.
  stale_alias <- c(
    file.path(out_dir, "all_strain_compare_results.xlsx"),
    file.path(out_dir, "all_raw_time_results.xlsx"),
    file.path(out_dir, "all_time_results.xlsx"),
  ...
  stale_alias <- stale_alias[file.exists(stale_alias)]
  if (length(stale_alias) > 0) file.remove(stale_alias)

  if (length(strain_result_list) == 0) {
  ...
  plot_df_list <- list()
  for (i in names(strain_result_list)) {
    raw_df <- strain_result_list[[i]]
    ...
    plot_df <- raw_df %>%
      ...
      filter(is.finite(fold_change_num), nzchar(strain)) %>%
      group_by(strain) %>%
      summarise(
  ...
  for (i in names(plot_df_list)) {
    ...
    meta_i <- header_cleaning(i, "_")
    mir <- if (nrow(meta_i) > 0 && "V2" %in% colnames(meta_i)) as.character(meta_i$V2[1]) else "GENE"
    t_lab <- if (nrow(meta_i) > 0 && "V3" %in% colnames(meta_i)) as.character(meta_i$V3[1]) else "time"
    ...
    if (length(sample_order) > 0) {
      ord <- unique(c(sample_order, plot_df$strain))
      ord <- ord[ord %in% plot_df$strain]
    } else {
      ord <- sort(unique(plot_df$strain))
    }
    # Always place the reference strain first on strain comparison figures when present.
    if (nzchar(reference_strain) && reference_strain %in% ord) {
      ord <- c(reference_strain, ord[ord != reference_strain])
    }
    plot_df$strain <- factor(plot_df$strain, levels = ord)
    plot_df <- plot_df %>% arrange(strain) %>% mutate(mean_label = sprintf("%.2f (n=%d)", mean_fc, n_points))
    x_labels <- setNames(as.character(plot_df$strain), as.character(plot_df$strain))

    raw_points <- strain_result_list[[i]] %>%
      mutate(
        fold_change_num = suppressWarnings(as.numeric(fold_change)),
        strain = factor(as.character(strain), levels = levels(plot_df$strain))
      ) %>%
      filter(is.finite(fold_change_num), !is.na(strain))

    p <- ggplot(plot_df, aes(x = strain, y = mean_fc)) +
      geom_col(width = 0.6, fill = "black") +
      geom_point(
        data = raw_points,
        aes(x = strain, y = fold_change_num),
        inherit.aes = FALSE,
      ...
      scale_y_continuous(
        limits = c(0, y_max_int + 0.5),
        breaks = integer_axis_breaks(0, y_max_int, 8),
        expand = expansion(mult = c(0, 0.03))
      ) +
      labs(x = "Strain", y = "Relative gene expression", title = paste(mir)) +
      ...
      theme(
        ...
        plot.margin = margin(t = 0.5, r = 4, b = 1.5, l = 0.5, unit = "lines"),
        panel.grid.major.x = element_blank()
      )

    # Stars: compare all strains against reference strain when available.
    ref <- reference_strain
    if (!nzchar(ref) || !(ref %in% as.character(plot_df$strain))) {
      ref <- as.character(plot_df$strain[1])
    }
    stars_map <- c()
    for (s in as.character(plot_df$strain)) {
      if (identical(s, ref)) next
      sub_df <- raw_points %>% filter(as.character(strain) %in% c(ref, s))
      pval <- safe_t_test_p(sub_df %>% mutate(p = as.character(strain)), group_col = "p")
      stars_map[s] <- p_to_stars(pval)
    }
    if (length(stars_map) > 0) {
      stars_df <- plot_df %>% filter(as.character(strain) %in% names(stars_map)) %>%
        mutate(stars = unname(stars_map[as.character(strain)]))
      p <- p + annotate("text", x = stars_df$strain, y = stars_df$stars_y + 0.05, label = stars_df$stars, size = 8)
    }
  ...
  # Remove stale fallback-only figures from previous runs in the same out_dir.
  old_fallback <- list.files(figure_dir, pattern = "^fallback_strain_.*\\.png$", full.names = TRUE)
  if (length(old_fallback) > 0) {
    file.remove(old_fallback)
  }

  for (nm in names(plot_list)) {
    nm_safe <- safe_file_component(nm)
    bar_n <- length(unique(as.character(plot_list[[nm]]$data$strain)))
    if (!is.finite(bar_n) || bar_n <= 0) bar_n <- 2
    save_plot_if_nonempty(
      plot_list[[nm]],
      file.path(figure_dir, paste0(nm_safe, "_result.png")),
      width = max(6, 2 * bar_n),
      height = 9
    )
  }
  ...
  for (m in miR_for_combine) {
    ...
    names(plots) <- names(plots) %>% header_cleaning("_") %>% pull(V3)
  ...
  for (nm in names(combine_plot_by_mir)) {
    ...
    bars_total <- sum(vapply(plot_list[grep(paste0("_", nm, "_"), names(plot_list))], function(p) {
      length(unique(as.character(p$data$strain)))
    }, integer(1)))
    if (!is.finite(bars_total) || bars_total <= 0) bars_total <- max(2, combine_plot_by_mir_n[[nm]] * 2)
    save_plot_if_nonempty(
      combine_plot_by_mir[[nm]],
      file.path(figure_dir, paste0("Gene_", nm_safe, "_combined.png")),
      width = max(6, 2 * bars_total),
      height = 9
    )
  }

  time_for_combine <- names(plot_list) %>% header_cleaning("_") %>% pull(V3) %>% unique()
  combine_plot_by_time <- list()
  combine_plot_by_time_n <- list()
  for (tt in time_for_combine) {
    plots <- plot_list[grep(paste0("_", tt, "$"), names(plot_list))]
    names(plots) <- names(plots) %>% header_cleaning("_") %>% pull(V2)
    combine_plot_by_time_n[[tt]] <- length(plots)
    combine_plot_by_time[[tt]] <- make_vertical_plot_grid(plots)
    if (is.null(combine_plot_by_time[[tt]])) next
  }
  for (nm in names(combine_plot_by_time)) {
    nm_safe <- safe_file_component(nm)
    h <- max(4, 4 * combine_plot_by_time_n[[nm]])
    bars_total <- sum(vapply(plot_list[grep(paste0("_", nm, "$"), names(plot_list))], function(p) {
      length(unique(as.character(p$data$strain)))
    }, integer(1)))
    if (!is.finite(bars_total) || bars_total <= 0) bars_total <- max(2, combine_plot_by_time_n[[nm]] * 2)
    save_plot_if_nonempty(
      combine_plot_by_time[[nm]],
      file.path(figure_dir, paste0("time_", nm_safe, "_combined.png")),
      width = max(6, 2 * bars_total),
      height = h
    )
  }
