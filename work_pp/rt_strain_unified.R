...
main <- function() {
  ...
  for (i in names(plot_df_list)) {
    ...
    x_labels <- setNames(
      paste0(
        "<span>", as.character(plot_df$strain), "</span><br>",
        "<span style='font-size:18pt;'>", plot_df$mean_label, "</span>"
      ),
      as.character(plot_df$strain)
    )
    ...
    p <- ggplot(plot_df, aes(x = strain, y = mean_fc)) +
      ...
      scale_y_continuous(
        limits = c(0, y_max_int),
        breaks = integer_axis_breaks(0, y_max_int, 8),
        expand = expansion(mult = c(0, 0.03))
      ) +
      labs(x = "Strain", y = "Fold Change", title = paste(mir, t_lab, "strain comparison")) +
      theme_bw() + gg_theme +
      theme(
  ...
  for (nm in names(combine_plot_by_time)) {
    ...
    save_plot_if_nonempty(combine_plot_by_time[[nm]], file.path(figure_dir, paste0("time_", nm_safe, "_combined.png")), width = 16, height = h)
  }
