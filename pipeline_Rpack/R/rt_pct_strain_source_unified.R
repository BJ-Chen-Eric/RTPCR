...
run_rt_pct_strain_source_unified <- function(args = NULL, base_dir = getwd(), ...) {
  normalize_arg_aliases <- function(x) {
    ...
    alias_map <- c(
      ...
      multi_compare_style = "multi-compare-style",
    ...
    nms[hit] <- unname(alias_map[nms[hit]])
    names(x) <- nms
    x
  }

  dot_args <- list(...)
  dot_args <- normalize_arg_aliases(dot_args)
  if (is.character(args)) {
    args <- parse_cli_args(args)
  }
  if (is.null(args)) args <- list()
  if (!is.list(args)) stop("`args` must be a list, a character CLI vector, or NULL.")
  args <- normalize_arg_aliases(args)
  if (length(dot_args) > 0) {
    args <- utils::modifyList(args, dot_args)
  }
  script_dir <- normalizePath(base_dir, mustWork = FALSE)
  if (isTRUE(args$help)) {
    print_usage()
    return(invisible(NULL))
  }
  ...
  message("[done] Excel outputs in: ", out_dir)
  message("[done] Figure outputs in: ", figure_dir)
  invisible(list(out_dir = out_dir, figure_dir = figure_dir, mode = analysis_mode))
}

rt_pct_strain_source_unified_cli <- function(args = commandArgs(trailingOnly = TRUE), base_dir = getwd()) {
  parsed <- parse_cli_args(args)
  run_rt_pct_strain_source_unified(args = parsed, base_dir = base_dir)
}
