...
print_usage <- function() {
  cat(
    "Usage:\n",
    "  Rscript rt_pct_strain_source_1227.R [options]\n\n",
    "Options:\n",
...
main <- function() {
  ...
  default_parent <- normalizePath(file.path(script_dir, "..", "RTpcr", "output"), mustWork = FALSE)
  default_raw <- normalizePath(file.path(script_dir, "..", "RTpcr", "raw"), mustWork = FALSE)
  ...
  if (length(time_points) > 2) {
    stop(
      "This script supports exactly two time points. Detected ",
      length(time_points),
      ": ",
  ...
  for (s1 in species) {
    for (g in miR) {
      ...
      control_pat <- paste0("_", gsub("([.|()\\^{}+$*?]|\\[|\\])", "\\\\\\1", control_time), "$")
      wide_reps_dt_c <- as.data.table(wide_reps_dt[grepl(control_pat, wide_reps_dt$p), ])
      ...
      wide_reps_dt_s <- as.data.table(wide_reps_dt[!grepl(control_pat, wide_reps_dt$p), ])
      ...
      wide_reps <- wide_reps %>%
        mutate(
          ...
          is_control = grepl(control_pat, p),
          control = mean(pre_fold_change[is_control], na.rm = TRUE),
          fold_change = 2^(-delta) / control
        ) %>%
        select(-species, -is_control)
  ...
  for (i in names(result_time_list)) {
    ...
    if (nrow(plot_df) < 2 || any(is.na(plot_df$sd_fc))) next
  ...
  for (m in miR_for_combine) {
    ...
    combine_plot_by_mir[[m]] <- make_square_plot_grid(plots)
    if (is.null(combine_plot_by_mir[[m]])) next
  }

  for (nm in names(combine_plot_by_mir)) {
    nm_safe <- safe_file_component(nm)
    save_plot_if_nonempty(combine_plot_by_mir[[nm]], file.path(figure_dir, paste0("Gene_", nm_safe, "_combined.png")), width = 16, height = 9)
  }
  ...
  for (s in strain_for_combine) {
    ...
    combine_plot_by_strain[[s]] <- make_square_plot_grid(plots)
    if (is.null(combine_plot_by_strain[[s]])) next
  }

  for (nm in names(combine_plot_by_strain)) {
    nm_safe <- safe_file_component(nm)
    save_plot_if_nonempty(combine_plot_by_strain[[nm]], file.path(figure_dir, paste0("strain_", nm_safe, "_combined.png")), width = 16, height = 9)
  }
